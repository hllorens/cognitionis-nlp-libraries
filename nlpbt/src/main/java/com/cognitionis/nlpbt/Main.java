...
public class Main {
    ...
    public static enum Action {
        ...
        DETECT_LANG, TOKENIZE, TAG, XML2TOK, TOK2XML, EVAL_ANNOT, TIMEX_PATTERN, TIMEX_NORM, TIMEX_RESOLVE, CANONICALIZE_PHRASELIST, NUMBERIZE, GET_NGRAMS, TRAIN_TAGGER, TEST_TAGGER
    ...
    public static void doAction(String action, String[] input_files,
        ...
        try {
            ...
            case TEST_TAGGER: {
                ...
                // TODO the model should contain the type of tagger so that we
                // create the adecuate class... or we can use an interface...
                for (final String file : input_files) {
                    final Baseline_MostFrequentTag tagger = new Baseline_MostFrequentTag();
    ...
    public static void doAction(String action, String input_text,
        ...
        try {
            ...
            case CANONICALIZE_PHRASELIST: {
                final String entity = getParameter(action_parameters, "entity");
                if (entity == null) {
                    throw new Exception(
                            "entity to be canonicalized --> parameter entity is required (e.g., entity=callmode)");
                }
                final String app_path = FileUtils.getApplicationPath();
                final PhraselistFile phra = new PhraselistFile(app_path
                        + File.separator
                        + "resources/canonicalization-phraselists"
                        + File.separator + entity + ".phraselist", false,
                        new Locale(lang), true, true, true);
                String cano = phra.getMapValue(input_text);
                if (cano == null) {
                    cano = "Not canonicalized";
                }
                System.out.println(input_text + " >>>>> " + cano);
                // TODO, make this happen with just a name it defines a folder
                // and a rule.phraselist
                // Canonicalizer instance = new Canonicalizer(new Locale(lang,
                // country), entity);
                // System.out.println(instance.canonicalize(input_text));
            }
                break;
            case TIMEX_PATTERN: {
                final Timek tiki = new Timek(new Locale(lang, country));
                System.out.println(tiki.getNormTextandPattern(input_text));
            }
                break;
            case TIMEX_NORM: {
                final TimexNormalizer tiki = new TimexNormalizer(new Locale(
                        lang, country));
                System.out.println(tiki.normalize(input_text));
            }
                break;
            case DETECT_LANG: {
                final TextCategorizer teka = new TextCategorizer();
            ...
            case NUMBERIZE: {
                ...
                final Numek nuki = new Numek(new Locale(lang, country));
