...
public final class Timex extends Element implements Cloneable {
    ...
    public void initialize_interval_values() {
        try {
            if (this.lower_value == null || this.upper_value == null) {
                ...
                this.isoclean = interval_values_arr[0];
                this.lower_value = interval_values_arr[1];
                this.upper_value = interval_values_arr[2];
            }
        } catch (Exception e) {
            System.err.println("Errors found (" + this.getClass().getSimpleName() + "):\n\t" + e.toString() + "\n");
            if (System.getProperty("DEBUG") != null && System.getProperty("DEBUG").equalsIgnoreCase("true")) {
        ...
        try {
            ...
            if (value.matches("(?i)[0-9]{4}-(WI|SP|SU|AU|FA|[QT](1|2|3|4)|H(1|2))")) {
                if (value.matches("(?i).*-(WI|Q1|H1|T1)")) {
                    ...
                    return year + "-01|" + year + "-01-01|" + year + "-02-" + february_last_day;
                }
                if (value.matches("(?i).*-(SP|Q2|T2)")) {
                    return year + "-03|" + year + "-03-01|" + year + "-05-31";
                }
                if (value.matches("(?i).*-(SU|Q3|H2|T3)")) {
                    return year + "-06|" + year + "-06-01|" + year + "-08-31";
                }
                if (value.matches("(?i).*-(AU|FA|Q4|T4)")) {
                    return year + "-09|" + year + "-09-01|" + year + "-12-31";
                }
            }
            ...
            if (value.matches("(?i)[0-9]{4}-[0-9]{2}-[0-9]{2}T(MO|AF|EV|NI|MI|NO)")) {
                ...
                if (value.matches("(?i).*TMO")) {
                    return date_string + "T05:00:00|" + date_string + "T05:00:00|" + date_string + "T11:59:59";
                } // NOON 12
                if (value.matches("(?i).*TNO")) {
                    return date_string + "T12:00:00|" + date_string + "T12:00:00|" + date_string + "T12:00:01";
                } // AFTERNOON 13
                if (value.matches("(?i).*TAF")) {
                    return date_string + "T12:00:01|" + date_string + "T12:00:01|" + date_string + "T17:59:59";
                } // DEPEND ON WORK BREAKS 17-18
                if (value.matches("(?i).*TEV")) {
                    return date_string + "T18:00:00|" + date_string + "T18:00:00|" + date_string + "T20:59:59";
                } // AFTER WORK... GOING BACK HOME...
                if (value.matches("(?i).*TNI")) {
                    return date_string + "T21:00:00|" + date_string + "T21:00:00|" + date_string + "T23:59:59";
                } // MIDNIGHT
                if (value.matches("(?i).*TMI")) {
                    return date_string + "T00:00:00|" + date_string + "T00:00:00|" + date_string + "T00:00:01";
                }
            }
            ...
            if (value.matches("(?i)[0-9]{4}-W[0-9]{2}(-WE)?")) {
                ...
                isoclean = df2.format(cal.getTime());
                lv = isoclean + "T00:00:00";
    ...
    /**
     * Simple function to calculate if a given year is a lap year
     *
     * @param year
     * @return
     */
